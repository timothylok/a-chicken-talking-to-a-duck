...
- **Current commands** (auto-synced from `COMMANDS` by the pre-commit hook — do not edit between the markers) — <!-- COMMANDS:BEGIN -->`SYSTEM_STATUS` (系統狀態)、`LIST_COMMANDS` (有咩指令)、`WEATHER_TODAY` (今日天氣)、`FUEL_PRICES` (油價)、`BUS_TIMES` (巴士)、`TIDE_TIMES` (潮汐)、`BIN_DAY` (幾時收垃圾)、`EARTHQUAKES` (地震)、`NEWS_HEADLINES` (新聞)、`MORNING_BRIEFING` (早晨)、`RESTART_ASR` (重啟語音系統)、`TRIGGER_DEPLOY` (重新部署，destructive)<!-- COMMANDS:END -->
