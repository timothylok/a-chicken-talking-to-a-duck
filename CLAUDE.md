...
- **Current commands** (auto-synced from `COMMANDS` by the pre-commit hook — do not edit between the markers) — <!-- COMMANDS:BEGIN -->`SYSTEM_STATUS` (系統狀態)、`LIST_COMMANDS` (有咩指令)、`WEATHER_TODAY` (今日天氣)、`FUEL_PRICES` (油價)、`BUS_TIMES` (巴士)、`TIDE_TIMES` (潮汐)、`BIN_DAY` (幾時收垃圾)、`RESTART_ASR` (重啟語音系統)、`TRIGGER_DEPLOY` (重新部署，destructive)<!-- COMMANDS:END -->
...
ASR service (AppEnvironmentExtra REG_MULTI_SZ under the service's Parameters key — write it with elevated `Set-ItemProperty`, not `nssm set`, whose quoting mangles values with spaces; currently set: HF_HOME, FUEL_LOCATION, BIN_ADDRESS):
