...
- **Monitoring** — scheduled task "VoiceOS Heartbeat" (`ops/heartbeat.ps1`, hourly — matches `ops/register_heartbeat.ps1`; an outage shorter than an hour can therefore pass unseen, as the 11-min one on 2026-09-07 would have) → healthchecks.io; logs at `asr/logs/service.log` and `logs/cloudflared.log`.
